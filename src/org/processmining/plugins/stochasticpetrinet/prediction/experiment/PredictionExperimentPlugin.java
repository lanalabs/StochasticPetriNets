...
public class PredictionExperimentPlugin {
	...
	/**
	 * number of iterations to split the log in: 10 is a usual tradeoff between performance and accuracy.
	 */
	...
	@Plugin(name = "Predict duration by Simulation (Simulated Experiment)", 
			...
			returnLabels = { "Prediction Results for each trace at multiple points in time" }, 
			returnTypes = { PredictionExperimentResult.class }, 
			userAccessible = true,
			help = "Predicts the remaining duration for a given trace and a model and time.")
	...
	@Plugin(name = "Predict durations (Real Experiment)", 
			parameterLabels = { "PetriNet", "Log" }, 
			returnLabels = { "Prediction / Real Value Pairs for each trace at multiple points in time" }, 
			returnTypes = { PredictionExperimentResult.class }, 
			userAccessible = true,
...
//			String caseString = caseId+SEPARATOR+(data.getCaseEndDate().getTime()-data.getCaseStartDate().getTime());
			sb.append(caseId).append(SEPARATOR).append(data.getCaseEndDate().getTime()-data.getCaseStartDate().getTime());
...
//				"Error (state bag)"+SEPARATOR+"Error (Pnet simple)"+SEPARATOR+"Error (Pnet constrained)";
				...
				long timePassed = prediction.getPredictionTimeDate().getTime()-prediction.getCaseStartDate().getTime();
				long realRemaining = prediction.getCaseEndDate().getTime()-prediction.getPredictionTimeDate().getTime();
				sb.append(SEPARATOR).append(predictionNo).append(SEPARATOR).append(timePassed).append(SEPARATOR).append(realRemaining);
...
//				caseString+=predictions+errors;
			...
			if (predictionErrors>0){
				System.out.println("Case "+caseId+" failed in: "+Arrays.toString(predictionfails)+" of " +predictionsMade+" cases.");
			}
			sb.append("\n");
//			caseString += "\n";
...
//				"Error (state bag 2)"+SEPARATOR+
	...
	/**
	 * Mines some different transition systems from a log and annotates them with time from the same log.
	 * states are determined by
	 * 1. last activity 
	 * 2. list of last activities
	 * 3. set of last activities
	 * 4. multiset of last activities
	 * 
	 * @param context
	 * @param trainingLog
	 * @return
	...
	private TimeTransitionSystemAnnotation[] getTimeTransitionSystemAnnotations(UIPluginContext context,
		...
		TSMinerInput input = new TSMinerInput(context, trainingLog, Arrays.asList(classifiers), new XEventAndClassifier(new XEventNameClassifier(),new XEventLifeTransClassifier()));
...
//		transitionSystemAnnotations[6] = getAnnotatedTransitionSystem(context, miner, input, analyzer, trainingLog);
	...
	private void changeSettings(TSMinerInput input, XEventClassifier[] classifiers, TSAbstractions abstractions, int horizon) {
	 ...
	 * @param predictionPairs a list of pairs containing the prediction in the first field and the real value in the second field.
	 * @return the MAPE (which has it's problems, if the real value is very small, i.e., near 0)  
	 */
	...
	/**
	 * 
	 * @param model SPN model learned from historical data (or real parametric model)
	 * @param log test log to test predictions made with the SPN model and transition systems 
	 * @param config {@link PredictionExperimentConfig} stores parameters used to make predictions
	...
	public PredictionExperimentResult predict(UIPluginContext context, StochasticNet model, TimeTransitionSystemAnnotation[] transitionSystemAnnotations, XLog log, PredictionExperimentConfig config, Marking initialMarking, double meanDuration){
		...
		for (int i = 0; i < transitionSystemAnnotations.length; i++){
			...
			if (transitionSystem instanceof EventPayloadTransitionSystem){
				...
				throw new IllegalArgumentException("TimeTransitionSystemAnnotation is not annotating an event payload transition system. Please annotate mined models only (for now).");
			}
		}
	...
	public long getProcessMeanDuration(StochasticNet model, XLog log, Marking initialMarking, PredictionExperimentConfig config) {
					...
					for (int i = 0; i < config.getMonitoringIterations(); i++){
						...
						if (realRemainingDuration >= 0){
							...
							for (int j = 0; j < transitionSystemAnnotations.length; j++){
								...
								org.processmining.models.graphbased.directed.transitionsystem.Transition transition = transitionSystem.getTransition(trace, subTrace.size()-1);
								if (transition != null){
									TimeStateAnnotation timeAnnotation = transitionSystemAnnotation.getStateAnnotation(transition.getTarget());
									predictions.getPredictionDates()[j+1] = new Date(Math.max(new Double(timeAnnotation.getRemaining().getAverage()).longValue()+realLastEventTime,currentTime.getTime()));
								}
							}
							predictions.getPredictionDates()[transitionSystemAnnotations.length+1] = new Date(predictedValueSimple.longValue());
							predictions.getPredictionDates()[transitionSystemAnnotations.length+2] = new Date(predictedValueConstrained.longValue());
